...
def translateDevices(domain, newVals, friendly, origin)
{
    def mapping =
        [
            ...
            media_player: [type: "Generic Component Media Player",      event: [[name: "switch", value: ["off", "standby"].contains(newVals[0]) ? "off":"on", type: origin, descriptionText:"${friendly} was turned ${["off", "standby"].contains(newVals[0]) ? "off":"on"} [${origin}]"],[name: "status", value: newVals[0], type: origin, descriptionText:"${friendly} status became ${newVals[0]}"],[name: "volume", value: newVals[1] ?: 'unavailable', descriptionText: "${friendly} volume was set to ${newVals[1] ?: 'unavailable'}%"],[name: "mute", value: newVals[2]? "muted":"unmuted", descriptionText: "${friendly} was ${newVals[2]? 'muted':'unmuted'}"],[name: "mediaInputSource", value: newVals[3], descriptionText: "${friendly} source was set to ${newVals[3]}"],[name: "supportedInputs", value: newVals[4]]], namespace: "community"],
