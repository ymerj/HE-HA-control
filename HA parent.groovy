	...
	if (newState?.state?.toLowerCase() == "unknown") return
        ...
        switch (domain) {
            ...
            case "humidifier":
                ...
                newVals += [humidifierMode, supportedModes, currentHumidity, targetHumidity]
                mapping = translateDevices(domain, newVals, friendly, origin)
                if (newVals[0] == "off") mapping = mapping.event.getAt(0) // remove updates not provided with the HA 'off' event json data
...
def translateDevices(domain, newVals, friendly, origin)
{
    def mapping =
        [
            ...
            humidifier: [type: "HADB Generic Component Humidifier",     event: [[name: "switch", value: newVals[0], type: origin, descriptionText:"${friendly} was turned ${newVals[0]} [${origin}]"],[name: "humidifierMode", value: newVals[1], descriptionText: "${friendly}'s humidifier is set to ${newVals[1]}"],[name: "supportedModes", value: newVals[2], descriptionText: "${friendly} supportedModes were set to ${newVals[2]}"],[name: "humidity", value: newVals[3], unit: "%", descriptionText:"${friendly} current humidity is ${newVals[3]}%"],[name: "targetHumidity", value: newVals[4], unit: "%", descriptionText:"${friendly} target humidity is set to ${newVals[4]}%"]], namespace: "community"],
