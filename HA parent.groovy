...
* 2.30   2026-08-20 Amithalp		   Fix button wrong value type causing replaceAll method error in control_panel
*/
...
def translateSensors(device_class, newVals, friendly, origin)
{
    def mapping =
        [
            ...
            current: [type: "HADB Generic Component Current Meter",           event: [[name: "amperage", value: newVals[0], unit: newVals[1] ?: "A", descriptionText:"${friendly} current is ${newVals[0]} ${newVals[1] ?: 'A'}"]], namespace: "community"],
...
void componentArmAway(ch) {
    if (logEnable) log.info("received arm away from ${ch.label}")
    executeCommand(ch, "alarm_arm_away")
}

void componentArmHome(ch) {
    if (logEnable) log.info("received arm home from ${ch.label}")
    executeCommand(ch, "alarm_arm_home")
}
