...
def translateDevices(domain, newVals, friendly, origin)
{
    def mapping =
        [
            ...
            humidifier: [type: "HADB Generic Component Humidifier",     event: [[name: "switch", value: newVals[0], type: origin, descriptionText:"${friendly} was turned ${newVals[0]} [${origin}]"],[name: "humidifierMode", value: newVals[1], descriptionText: "${friendly}'s humidifier is set to ${newVals[1]}"],[name: "supportedModes", value: newVals[2], descriptionText: "${friendly} supportedModes were set to ${newVals[2]}"],[name: "maxHumidity", value: newVals[3] ?: 'none', descriptionText:"${friendly} min humidity is ${newVals[4] ?: 'none'}"],[name: "minHumidity", value: newVals[4] ?: 'none', descriptionText:"${friendly} min humidity is ${newVals[4] ?: 'none'}"],[name: "humidity", value: newVals[5] ?: 'none', descriptionText:"${friendly} current humidity is ${newVals[5] ?: 'none'}"],[name: "targetHumidity", value: newVals[6] ?: 'none', descriptionText:"${friendly} target humidity is set to ${newVals[6] ?: 'none'}"]], namespace: "community"],
